...
from langchain_aws import ChatBedrock  # ✅ Updated to use correct Chat model
...
def get_claude_llm():
    ...
    return ChatBedrock(
        # Add your own Inference profile ID
        model_id=Haiku_Id,
...
def get_response_llm(llm, vectorstore_faiss, query):
    ...
    # ✅ Updated deprecated `__call__` to `.invoke`
...
if __name__ == "__main__":
    ...
    if st.button("Exit App"):
        st.warning("The app is now stopped. You may close this tab manually.")
        st.stop()

    with st.sidebar:
        st.markdown("---")
        if st.button("❌ Exit App"):
            st.warning("🛑 App stopped. Please close this tab manually.")
            st.stop()